...
Visual.renderer.pie = function (dataFrame, options) {
    ...
    // Format data
    let categoryName = Object.getOwnPropertyNames(options.fnCategories(dataFrame.data[0]))[0];
    let valueName = Object.getOwnPropertyNames(options.fnValues(dataFrame))[0];
    let data = dataFrame.group(
        options.fnCategories,
        options.fnValues
...
Visual.renderer.scatter = function (dataFrame, options) {
    ...
    // Get column names
    let xColumnName = Object.getOwnPropertyNames(options.fnXValues(dataFrame.data[0]))[0];
    let yColumnName = Object.getOwnPropertyNames(options.fnYValues(dataFrame.data[0]))[0];
    ...
    // Add X axis
    let minXValue = d3.min(dataFrame.data, function (d) { return +d[xColumnName] });
    let maxXValue = d3.max(dataFrame.data, function (d) { return +d[xColumnName] });
    ...
    // Add Y axis
    let minYValue = d3.min(dataFrame.data, function (d) { return +d[yColumnName] });
    let maxYValue = d3.max(dataFrame.data, function (d) { return +d[yColumnName] });
