...
class sw {
    ...
    static help() {
        ...
        // Get classes
        let classes = sw.documentation.operations.map((o) => o.class).filter((value, index, self) => { return self.indexOf(value) === index });

        // Display Index
        classes.forEach((c) => {
            html += `<h4>${c}</h4>`;
            let operations = sw.documentation.operations.filter((o) => o.class === c);
            operations.sort((a, b) => { return a.name > b.name ? 1 : -1 });
    ...
    <div class="helpOperationBody">
        ...
        <div>${o.description}</div>`;
            ...
            signatures.forEach((s) => {
                ...
                let signature = `.${o.name}(${s.map((sig) => sig.parameterName).join(', ')})`;
...
documentation.operations.push({
    ...
    async: false,
    returns: '',
    description: 'Creates a layout structure on the output pane allowing multiple visuals to be placed onto it.',
    signatures: [
        [
            {
                parameterName: 'panels',
                parameterType: 'string',
                parameterDescription: 'A string id value for the pane to create. Specifying the id alone will result in a single panel being created in the output pane.'
            }
        ],
        [
            {
        ...
        [
            {
                parameterName: 'panels',
                parameterType: 'Array',
                parameterDescription: 'An array of panels. These can be either string values or object configurations. Multiple panels will then be created.'
            }
        ],
        [
            {
            ...
            },
            {
                ...
                parameterType: 'string',
                parameterDescription: 'The direction / alignment of the panels within their parent. Can be set to \'horizontal\' or \'vertical\'.'
            }
        ],
        [
            {
            ...
            },
            {
                parameterName: 'direction',
                parameterType: 'string',
                parameterDescription: 'Either the value [vertical] or [horizontal]. This defines how the layout will be drawn.'
            },
            {
                parameterName: 'parentId',
                parameterType: 'string',
                parameterDescription: 'The id of the parent container to place the panels. Must be an existing element. If omitted, or null entered, the main layout will be added to the main output pane.'
            }
        ]
    ],
    examples: [
...
UI.layout = function (panels, direction = 'horizontal', parentId = null) {
    ...
    */
    ...
    let total = panels.reduce((acc, cur) => { return acc + cur.size }, 0);
        ...
        [
            {
                ...
                parameterType: 'Any',
                parameterDescription: 'The content to write to the output. The content will be rendered in different ways depending on the type of object:<ul><li><b>string</b> - content rendered as Html content</li><li><b>Node</b> - Html node element placed directly into output. Note that the Visuals class returns Node objects.</li><li><b>object</b> - object content stringified prior to rendering</li><li><b>number</b> - numeric and other primitive types written directly.</li></ul>'
            }
        ], [
            {
                parameterName: 'content',
            ...
            },
            {
                parameterName: 'id',
                parameterType: 'string',
                parameterDescription: 'The id of the parent pane to add the content. If the id is omitted, the content will be rendered to the main output pane.'
            }
        ], [
            {
                parameterName: 'content',
            ...
            },
            {
                ...
                parameterDescription: 'If set to true, the content will be appended to existing content in the pane. Otherwise, the existing content will be replaced.'
            }
        ]
    ],
    examples: [
...
UI.content = function (content, id = null, append = false) {
...
Element.prototype.fit = function () {
    if (this.id !== 'output') {
        ...
        if (elInner) {
            ...
            // till fits.
            let tries = 0;
            while (elInner.scrollWidth > elInner.clientWidth || elInner.scrollHeight > elInner.clientHeight) {
                elInner.style.zoom = elInner.style.zoom * .99;
                tries++;
                if (tries > 10) {
                    break;
                }
